...
import rotationRequestSchema from "../../../contracts/private-generate-rotation-request.schema.json" with {
  type: "json",
};
...
export interface RotationRequestValidator {
...
export function createRotationRequestValidator(): RotationRequestValidator {
  ...
  addFormats(ajv);
  const validateRequest = ajv.compile(normalizeRefs(rotationRequestSchema));

  return {
    validate(value: unknown): ValidationIssue[] {
      return validateRequest(value) ? [] : issues(validateRequest.errors);
    },
  };
}
