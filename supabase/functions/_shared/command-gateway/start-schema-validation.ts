...
import startRequestSchema from "../../../contracts/private-start-expedition-request.schema.json" with {
  type: "json",
};
...
export interface StartRequestValidator {
...
export function createStartRequestValidator(): StartRequestValidator {
  ...
  addFormats(ajv);
  const validateRequest = ajv.compile(normalizeRefs(startRequestSchema));
  return {
    validate(value: unknown): ValidationIssue[] {
